...
const PORTRAIT_W = 9700, LANDSCAPE_W = 14200;
...
const para = (t, o = {}) => new Paragraph({spacing: {after: o.after ?? 110, line: 276},
  alignment: o.align, indent: o.indent, keepNext: o.keepNext,
  children: runs(t, {size: o.size ?? 21, color: o.color ?? "000000", bold: o.b, italics: o.i})});

function heading(t, level) {
  const size = {1: 34, 2: 26, 3: 22}[level];
  const hl = {1: HeadingLevel.HEADING_1, 2: HeadingLevel.HEADING_2, 3: HeadingLevel.HEADING_3}[level];
  return new Paragraph({heading: hl, keepNext: true,
    spacing: {before: level === 1 ? 120 : 240, after: level === 1 ? 160 : 100},
...
const bullet = (t, lvl = 0) => new Paragraph({numbering: {reference: "b", level: lvl},
  spacing: {after: 70, line: 264}, children: runs(t, {size: 21})});

const hanging = (label, t) => new Paragraph({spacing: {after: 70, line: 264},
  indent: {left: 420, hanging: 300},
  children: [new TextRun({text: label + "\t", font: F, size: 21, bold: true, color: DK}), ...runs(t, {size: 21})],
...
function callout(label, lines, width) {
  ...
  for (const l of lines) {
    if (/^- \[ \] /.test(l)) kids.push(new Paragraph({spacing: {after: 50}, indent: {left: 300, hanging: 300}, children: [new TextRun({text: "☐\t", font: F, size: 21}), ...runs(l.slice(6), {size: 21})]}));
    else if (/^- /.test(l)) kids.push(new Paragraph({spacing: {after: 50}, indent: {left: 300, hanging: 220}, children: [new TextRun({text: "•\t", font: F, size: 21, color: col}), ...runs(l.slice(2), {size: 21})]}));
    else if (/^\d+\. /.test(l)) { const m = l.match(/^(\d+\.) (.*)$/); kids.push(new Paragraph({spacing: {after: 50}, indent: {left: 360, hanging: 360}, children: [new TextRun({text: m[1] + "\t", font: F, size: 21, bold: true, color: col}), ...runs(m[2], {size: 21})]})); }
    else if (l.trim()) kids.push(new Paragraph({spacing: {after: 60, line: 264}, children: runs(l, {size: 21})}));
  }
  return [new Table({columnWidths: [width], width: {size: width, type: WidthType.DXA},
    rows: [new TableRow({cantSplit: true, children: [new TableCell({width: {size: width, type: WidthType.DXA},
      ...
      children: kids})]})]}), new Paragraph({spacing: {after: 80}, children: []})];
}
...
function table(rows, width) {
  ...
  const mk = (c, i, head, zebra) => new TableCell({width: {size: widths[i], type: WidthType.DXA},
    ...
    children: [new Paragraph({alignment: !head && i > 0 && numeric(c) ? AlignmentType.RIGHT : undefined,
      children: runs(c, {size: 19, bold: head || undefined, color: head ? "FFFFFF" : "000000"})})]});
...
function canvas(lines, width) {
  ...
  const cell = (k, cs, rs, fill) => {
    ...
    return new TableCell({columnSpan: cs, rowSpan: rs, width: {size: unit * cs, type: WidthType.DXA},
      ...
      children: [new Paragraph({spacing: {after: 60}, children: [new TextRun({text: b.title.toUpperCase(), font: F, size: 16, bold: true, color: ACC})]}),
        ...b.items.map(t => new Paragraph({spacing: {after: 30, line: 240}, indent: {left: 140, hanging: 140},
          children: [new TextRun({text: "• ", font: F, size: 16, color: ACC}), ...runs(t, {size: 16})]}))]});
  };
  return [new Table({columnWidths: Array(10).fill(unit), width: {size: unit * 10, type: WidthType.DXA},
    rows: [
      new TableRow({children: [cell("kp", 2, 2), cell("ka", 2, 1), cell("vp", 2, 2, "EAF4F1"), cell("cr", 2, 1), cell("cs", 2, 2)]}),
      new TableRow({children: [cell("kr", 2, 1), cell("ch", 2, 1)]}),
      new TableRow({children: [cell("cost", 5, 1, "F7F7F7"), cell("rev", 5, 1, "F7F7F7")]}),
    ]})];
}
...
function convert(md) {
  ...
  for (let i = 0; i < L.length; i++) {
    ...
    if (h) {
      ...
      kids.push(heading(h[2], h[1].length)); continue;
    }
    if (l.startsWith(":::")) {
      flushPara();
    ...
    let m;
    if ((m = l.match(/^- \[ \] (.*)$/))) { flushPara(); kids.push(new Paragraph({spacing: {after: 70, line: 264}, indent: {left: 420, hanging: 320}, children: [new TextRun({text: "☐\t", font: F, size: 22}), ...runs(m[1], {size: 21})]})); continue; }
...
function build(md, out, footerText) {
  ...
  const doc = new Document({
    ...
    sections: secs.map(s => ({
      properties: {page: {size: s.orient === "landscape" ? {orientation: PageOrientation.LANDSCAPE, width: 16838, height: 11906} : {width: 11906, height: 16838},
        margin: {top: 900, bottom: 900, left: s.orient === "landscape" ? 1300 : 1100, right: s.orient === "landscape" ? 1300 : 1100}}},
